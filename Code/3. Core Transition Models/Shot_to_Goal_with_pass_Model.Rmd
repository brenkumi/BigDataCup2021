...
shot_data = otters_data %>%
  ...
  # Unnest
  unnest() %>% ungroup() %>%
  # Create a decaying time value metric for time since the entry
  ...
  # Create a decaying time value metric for time since the last pass
  mutate(pass_time_decay = case_when(
    last_pass_time == 0 ~ 1,
    last_pass_time < 10 ~ 1 - log10(last_pass_time),
    last_pass_time >= 10 ~ 0,
    TRUE ~ 0
  )) %>%
  # Create a decaying time value metric for time since the last shot
...
# Filter for shots/goals that were preceded by a pass as well as our specified strength state
shot_data = shot_data %>%
...
saveRDS(mesh.t, here::here(paste("Models/Meshes/Shot_to_Goal_with_pass", strength, "Time_Mesh.Rda", sep = "_")))
```
...
X = shot_data %>%
  ...
  select(offensive_score_diff, offensive_strength_diff, period, pass_time_decay, entry_time_decay, shot_time_decay, shot_count,
         detail_3, detail_4, is_first_shot, is_prev_event_pass, time_since_entry, pass_distance, angle_change, from_behind_net)
```
...
model_formula = as.formula(paste0("y ~ -1 + Intercept ", 
                                  ...
                                  " + shot_time_decay + shot_count",
                                  " + detail_3 + detail_4 + entry_time_decay",
