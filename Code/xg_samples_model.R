...
plot(Mesh) +point(model_coords)


# Set the knots
knots = c(0,2, 5,10, max(pass_frame$time_since_entry.x))
...
model_spde = inla.spde2.pcmatern(mesh = Mesh, prior.range = c(35, 0.7), prior.sigma = c(0.5, 0.5))
...
pass_not_0 <- pass_frame[which(pass_frame$xg_0 ==0),]


model_A_matrix2 = inla.spde.make.A(Mesh, loc = cbind(pass_not_0$same_direction_x, pass_not_0$same_direction_y), n.group = length(knots), group = pass_not_0$time_since_entry.x, group.mesh = time_Mesh)
...
model_stack2 = inla.stack(
  ...
  # Add our response variable
  data = list(y = pass_not_0$Expected_Goals), 
  ...
  effects = list(
    ...
    # Intercept, repeat 1 n times
    Intercept = rep(1, nrow(pass_not_0)),
...
model_map = gg_OZ(model2, Mesh, Groups = 5) + scale_fill_viridis_d()

model_map


plot(Mesh) + points(model_coords[which(pass_frame$xg_0 ==0),])
