...
source(here::here("Code/2. Functions/Plotting Functions.R"))
```
...
For visualization purposes, we can add in "latent movements" between each observed play. This takes a while to run (roughly 8 minutes for me), but upon completion we can use ggplot
...
## STEP 1: LABEL ALL PUCK RECOVERY AND TAKEAWAY EVENTS AS "RECOVERY" (FOR NOW) ##
...
otters_data_clean_v10 = otters_data_clean_v9 %>%
  ...
  mutate(data = case_when(
    ...
    on_offence == 0 & length > 1 ~
      ...
      map(data, ~rbind(head(.x, 1), tail(.x, 1)) %>%
            # Relabel first plays after puck acquisition by defensive team as turnovers
            mutate(Event = ifelse(Event %in% c("Play", "Incomplete Play", "Recovery"), "Turnover", Event)) %>%
...
## STEP 5: ADD IN RECOVERIES BETWEEN SHOTS AND TURNOVERS BEFORE EXITS BY DEFENSIVE TEAMS ##
otters_data_clean_v11 = otters_data_clean_v10 %>%
  ...
  mutate(to_pass = ifelse((Event %in% c("Play", "Incomplete Play")) & (lag(Event) == "Turnover"), 1, 0)) %>%
...
otters_data_clean_v14 = otters_data_clean_v13 %>%
  ...
  mutate(event = ifelse(event == "Dump In/Out" & prev_event != "Zone Entry" & detail_1 == "Retained", "Play", event)) %>%
  mutate(event = ifelse(event == "Dump In/Out" & prev_event != "Zone Entry" & detail_1 == "Lost", "Incomplete Play", event)) %>%
  mutate(detail_1 = ifelse(event %in% c("Play", "Incomplete Play") & detail_1 %in% c("Retained", "Lost"), "Indirect", detail_1)) %>%
  # Remove recoveries after a possession play
  filter(!(event == "Recovery" & prev_event %in% c("Play", "Incomplete Play", "Recovery"))) %>%
  ...
  group_by(entry_id) %>%
  nest() %>%
  mutate(data = map(data, ~.x %>% mutate(prev_event = lag(event), next_event = lead(event)))) %>%
  unnest() %>%
  ...
  mutate(len = map(data, ~nrow(.x)) %>% as.numeric()) %>%
...
otters_data_clean_v15 = otters_data_clean_v14 %>%
  ...
  mutate(is_prev_event_pass = ifelse(event %in% c("Shot", "Goal") & prev_event %in% c("Play", "Incomplete Play"), 1, 0)) %>%
  ...
  mutate(last_pass_x = ifelse(is_prev_event_pass == 1, lag(same_direction_x), NA),
         ...
         last_pass_time = ifelse(is_prev_event_pass == 1, c_seconds - lag(c_seconds), NA)) %>%
  ...
  mutate(detail_1 = ifelse(detail_1 == "Dumped" & next_event %in% c("Incomplete Play", "Play"), "Carried", detail_1)) %>%
  # Add in score effect
  mutate(score_effect = case_when(
...
otters_data_clean_v16 = otters_data %>%
  ...
  mutate(Player = ifelse(event == "Turnover" & prev_event == "Play", lag(Player_2), Player))


## STEP 2: DETERMINE WHICH EVENTS ARE ASSIGNED TO OTTERS ##

# Get a list of all Otters in the data
otters_players = otters_data %>%
...
# Create an event ID
otters_data_clean_v17
  mutate(event_id = 1:nrow(.))

# Get pass counts
pass_data = otters_data_clean_v17 %>%
...
otters_data_clean_v18 = otters_data_clean_v17 %>%
...
```{r save3}
write.csv(otters_data_clean_v20, here::here("Data/otters_data_for_modelling.csv"), row.names = FALSE)
```
